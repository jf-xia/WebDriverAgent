...
fetch_source() {
    ...
    if [[ ! -s "$output_file" ]]; then
        echo -e "${RED}source 为空: $output_file${NC}" >&2
        return 1
    fi

    printf '%b\n' "${GREEN}source 已保存: $output_file${NC}" >&2
}
...
fetch_screenshot() {
    ...
    if [[ ! -s "$output_file" ]]; then
        printf '%b\n' "${RED}截图文件为空: $output_file${NC}" >&2
        return 1
    fi

    printf '%b\n' "${GREEN}截图已保存: $output_file${NC}" >&2
}
...
main() {
    ...
    if [[ -n "$PYTHON_SCRIPT" ]] && [[ -f "$PYTHON_SCRIPT" ]]; then
        if python3 "$PYTHON_SCRIPT" "$source_file" "$yaml_file" --remove-decoration 2>/dev/null; then
            printf '%b\n' "${GREEN}source 已转换为 YAML: $yaml_file${NC}" >&2
        else
            printf '%b\n' "${YELLOW}YAML 转换失败，回退到 JSON: $source_file${NC}" >&2
            yaml_file="$source_file"
        fi
    else
        printf '%b\n' "${YELLOW}未找到 YAML 转换脚本，直接使用 JSON: $source_file${NC}" >&2
        yaml_file="$source_file"
    fi
    ...
    printf '%b\n' "${GREEN}本次 snapshot 完成: source=$yaml_file screenshot=$screenshot_file${NC}" >&2
