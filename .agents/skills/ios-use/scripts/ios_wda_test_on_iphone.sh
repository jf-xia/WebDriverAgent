...
# 脚本专属默认值
PROJECT_PATH="$PROJECT_ROOT/WebDriverAgent.xcodeproj"
...
main() {
    ...
    # 1. 获取设备 UDID
    log_info "1/5 获取设备 UDID"
    local udid
    if ! udid=$(get_device_udid); then
        log_error "未找到连接的 iPhone 设备"
        exit 1
    fi
    log_success "设备 UDID: $udid"

    # 2. 验证工具可用性
    log_info "2/5 验证工具可用性"
    ...
    # 3. 清理旧的 tmux session
    log_info "3/5 清理旧 tmux session"
    cleanup_old_tmux_sessions

    # 4. 启动 iproxy
    log_info "4/5 启动 iproxy"
    if ! start_iproxy "$udid" "$PORT" "$PORT"; then
        log_error "iproxy 启动失败"
        exit 1
    fi

    # 5. 启动 xcodebuild
    log_info "5/5 启动 xcodebuild"
    ...
    log_success "已在 tmux 会话 '$session_name' 中启动 xcodebuild"
    log_info "查看日志: tmux attach -t $session_name"
    log_info "iproxy 日志: ${LOG_DIR}/iproxy.log"
    log_info "wda 日志: $log_file"
    log_success "启动命令已提交，总耗时 $(elapsed_seconds "$start_time")s"
    printf 'iproxy: localhost:%s -> device:%s\n' "$PORT" "$PORT"
    printf 'WDA 端点: http://127.0.0.1:%s\n' "$PORT"
}
